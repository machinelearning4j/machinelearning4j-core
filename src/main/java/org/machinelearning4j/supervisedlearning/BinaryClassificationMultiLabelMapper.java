 ...
 */
public abstract class BinaryClassificationMultiLabelMapper<C> implements MultiNumericLabelMapper<C> {
	...
	public double[][] getLabelValues(List<C> labels) {
		...
		for (C label : labels)
		{
			labelValues[labelValueIndex++] = getLabelAsVector(label);
		}
		return labelValues;
	}
	
	protected abstract double[] getLabelAsVector(C label);
